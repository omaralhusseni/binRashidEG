...
export default function BusSetup() {
  ...
  useEffect(() => {
    ...
    if (!video || !videoDesktop || !servicesVideo || !servicesVideoDesktop || !thirdSection) return;

    // Determine which videos to use based on screen size
    const isDesktop = window.innerWidth >= 1024; // lg breakpoint
    const activeVideo = isDesktop ? videoDesktop : video;
    const activeServicesVideo = isDesktop ? servicesVideoDesktop : servicesVideo;
    ...
    const createTimelines = () => {
      if (isNaN(activeVideo.duration) || activeVideo.duration === 0 ||
        isNaN(activeServicesVideo.duration) || activeServicesVideo.duration === 0) return;
      ...
      mainTl.to(activeServicesVideo, { opacity: 1, duration: 0.0625 }, "-=0.0625");
    ...
    return () => {
      ...
      activeServicesVideo.removeEventListener("loadedmetadata", createTimelines);
    };
  }, []);
  ...
  return (
    <main ref={container} className="w-full home relative kkfont normal">
      ...
      <section className="section1 w-screen h-screen bg-white z-0 overflow-hidden text-3xl">
        ...
        <div className="text kkfont normal w-700 text-left absolute mt-24 ml-4 text-3xl w-52 z-10 lg:text-5xl lg:w-screen bg-white lg:p-4 lg:ml-0 lg:left-0">
          <p>Business Setup In Emirates </p>
          <p className="text-[#1D7CB9] mt-2">And In Saudi Arabia</p>
        </div>

        <img className="z-10 w-full h-full object-cover object-center" src="map.png" />
      </section>
